...
(function () {
    ...
    function storeMedia(map, key, info) {
        const prev = map.get(key);
        if (!prev || info.variants.length >= prev.variants.length)
            map.set(key, info);
    }
    window.addEventListener("message", (e) => {
        if (e.source !== window)
            return;
        const d = e.data;
        if (!d || d.__xvdl !== true || d.kind !== "media" || !d.tweetId)
            return;
        const info = {
            mtype: d.mtype || "video",
            variants: d.variants || [],
        ...
        if (d.mediaId)
            storeMedia(mediaIdMap, d.mediaId, info);
    });
    // ---------- メッセージ ----------
    function send(msg) {
    ...
    function getMediaIdFromVideo(video) {
        const poster = video.getAttribute("poster") || "";
        let id = extractMediaId(poster);
        if (!id) {
            const c = video.closest('[data-testid="videoComponent"]') ||
                video.closest('[data-testid="videoPlayer"]') ||
                video.parentElement;
            const img = c && c.querySelector('img[src*="twimg.com"]');
            if (img)
                id = extractMediaId(img.getAttribute("src"));
        }
        return id;
    }
    function getMedia(video) {
        ...
        if (!info) {
            ...
            if (mid)
                info = mediaIdMap.get(mid) || null;
        }
        return { tweetId, user: getScreenName(video), info };
    }
    function makeName(user, tweetId, ext, suffix) {
    ...
    // ---------- 保存処理 ----------
    function saveMp4(user, tweetId, variant) {
    ...
    function origImageUrl(src) {
        try {
            ...
            const ext = fmt === "png" ? "png" : fmt === "webp" ? "webp" : fmt === "gif" ? "gif" : "jpg";
            return {
    ...
    async function convertToGif(mp4Url, opts, onProgress) {
        ...
        try {
            ...
            await new Promise((res, rej) => {
                ...
                video.addEventListener("error", () => rej(new Error("動画の読み込みに失敗")));
            ...
            if (!g || !g.GIFEncoder)
                throw new Error("gifencが読み込まれていません");
    ...
    let gifBusy = false;
    async function saveGif(user, tweetId, info) {
        if (gifBusy) {
    ...
    document.addEventListener("click", (e) => {
        ...
        if (!target || !target.closest(".xvdl-wrap"))
            closeAllMenus();
    }, true);
    function menuItem(label, disabled, onClick) {
    ...
    function fillMenu(video, menu) {
        ...
        if (info.mtype === "animated_gif" && info.variants.length) {
            menu.appendChild(menuItem("🎞 GIF として保存", false, () => {
                void saveGif(m.user, m.tweetId, info);
            }));
        }
        info.variants.forEach((v, i) => {
            const label = v.w && v.h ? "⬇ MP4  " + v.w + "×" + v.h : "⬇ MP4  #" + (i + 1);
            menu.appendChild(menuItem(label, false, () => saveMp4(m.user, m.tweetId, v)));
        });
        if (!info.variants.length) {
            menu.appendChild(menuItem(info.hls ? "HLS形式のみのため保存できません" : "保存可能な動画なし", true));
        }
    }
    function defaultAction(video) {
        const m = getMedia(video);
        if (!m.info) {
    ...
    function buildUI(video) {
        ...
        if (!container)
            return;
        if (container.querySelector(":scope > .xvdl-wrap"))
            return;
    ...
    function buildImageUI(img) {
        const container = img.closest('[data-testid="tweetPhoto"]') ||
            img.parentElement;
        if (!container)
            return;
        if (container.querySelector(":scope > .xvdl-wrap"))
        ...
        if (!/pbs\.twimg\.com\/media\//.test(img.currentSrc || img.src))
            return;
        if (getComputedStyle(container).position === "static") {
            container.style.position = "relative";
        }
